    ...
    return {
      ...
      async run(prompt, hooks = {}) {
        ...
        try {
          ...
          receipt.metadata = { ...receipt.metadata, profileDigest: profile.digest, executableSha256: profile.executableSha256, runId: runKey, pid: result.pid, terminationCause: result.cause, counts: result.counts, synthetic: profile.synthetic, externalEffects: result.cause === "EXIT" ? "NONE_DECLARED" : "UNKNOWN" };
          if (!receipt.ok) {
