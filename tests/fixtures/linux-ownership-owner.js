...
const { identity, inventory } = require("./linux-ownership-host");
...
const observe = () => {
  for (const item of inventory(token)) if (item.namespace && item.namespace !== hostNamespace) namespaces.add(item.namespace);
