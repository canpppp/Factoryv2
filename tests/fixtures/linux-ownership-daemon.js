...
if (config.mode === "cleanup-unknown") {
  // Fault only the observation of this fixture's newly spawned backend group.
  // Actual spawn, signals, containment, adapter and durable consumers are real.
  ...
  childProcess.spawn = function(command, ...args) {
    ...
    if (command === "/usr/bin/bwrap") ownedPid = child.pid;
    return child;
  };
  process.kill = function(pid, signal) {
    if (ownedPid && pid === -ownedPid && signal === 0) throw Object.assign(new Error("synthetic observation unavailable"), { code: "EPERM" });
...
const poll = setInterval(() => {
  ...
  const attempt = journal.load(config.root).events.findLast((event) => event.type === "worker.attempt.finished");
  if (result && attempt && !reported.has(attempt.jobId)) { reported.add(attempt.jobId); process.send({ type: "settled", result, attempt }); }
}, 10);
